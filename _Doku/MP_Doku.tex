...
\begin{figure}[h]
	\centering 
	\includegraphics[width=0.7\columnwidth]{Diag_2f} 
...
Die Worker-Prozesse können nun feststellen, ob es sich bei einzelnen/zusammenhängenden Pixeln (im Folgenden als Figur bezeichnet) um Rechtecke handelt oder nicht. In der Folgenden Grafik, werden Figuren, die kein Rechteck darstellen mit einem \textit{X} dargestellt.
...
Wenn ein Rechteck in einem Teil-Raster zu einem Rand der Teil-Rasters reicht und an diesem Rand im gesamten Raster ein weiteres Raster angrenzt, so könnte die Figur im angrenzenden Teil-Raster fortgesetzt werden. In diesem Fall ist also durch das Verarbeiten des Teil-Rasters keine Aussage darüber möglich, ob es sich um ein tatsächlich um ein Rechteck handelt. Entsprechende Rechtecke (Im Folgenden als potentielle Rechtecke) werden in der folgenden Grafik als \textit{?} gekennzeichnet.\\
...
Die finale Überprüfung der potentiellen Rechtecke, muss also vom Master-Prozess übernommen werden.
...
Wie dabei Arbeits-Aufteilung und Kommunikation zwischen den Prozessen verläuft sei im Folgenden als BPMN-Daigramm dargestellt.
