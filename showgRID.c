...
void checkPotentialRectangle(char *gridPointer, int gridColumns, int line, int isUpperLine){
    ...
    for (i=0; i < gridColumns; i++){
        if (gridPointer[gridColumns * line + i] == 4){
            ///////////////
            printf("entered the loop at %d/%d\n",line,i);
            int startcolumn = i;
            int endcolumn = i;
            //get end of startline
	...
	if(rank == 0){
        ...
        if (argv[1]){
            ...
            if (file) {
                ...
                //for the moment: leave out 0-Process
                ...
                for (reciever = 1; reciever <= workingProcesses ; reciever++){
                    ...
                    printf("reciever: %d -> subgridLines: %d gridSize: %ld -> wanna send %ld\n",reciever, subgridLines, gridSize ,subgridLines*gridSize);
	    ...
	    if (rank <= gridSize[1]){            
            ...
            //printCharArray(&grid[0],gridSize[0]*gridSize[1]);
			printf("rank %d: gS0:%ld gS1:%ld\n",rank,gridSize[0],gridSize[1]);
			//recieve grid array
            MPI_Recv(&grid[0],gridSize[0]*gridSize[1],MPI_BYTE,0,0,MPI_COMM_WORLD,&status);
			printf("process %d recved sucsessful\n",rank);
